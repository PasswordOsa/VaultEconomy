package org.osa678.vaulteconomy.Comandos;
...
import org.osa678.vaulteconomy.Gancho.VaultApi;
import org.osa678.vaulteconomy.VaultEconomy;
...
public final class VaultComandos implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        if ("view".equals(param)) {
                            double balance = VaultApi.getBalance(target);

                            sender.sendMessage(ChatColor.GOLD + target.getName() + "'s balance: " + VaultApi.formatCurrencySymbol(balance));
                            ...
                            if ("take".equals(param)) {
                                String errorMessage = VaultApi.withdraw(target, amount);

                                if (errorMessage != null && !errorMessage.isEmpty())
                                    sender.sendMessage(ChatColor.RED + "Error: " + errorMessage);
                                else
                                    sender.sendMessage(ChatColor.RED + "Took " + VaultApi.formatCurrencySymbol(amount) + " from " + target.getName() + "' account.");

                            } else {
                                String errorMessage = VaultApi.deposit(target, amount);

                                if (errorMessage != null && !errorMessage.isEmpty())
                                    sender.sendMessage(ChatColor.RED + "Error: " + errorMessage);
                                else {
                                    sender.sendMessage(ChatColor.GREEN + "Gave " + VaultApi.formatCurrencySymbol(amount) + " to " + target.getName() + "' account.");
                                }
                            }
                ...
                }.runTask(VaultEconomy.getInstance());
            }
        }.runTaskAsynchronously(VaultEconomy.getInstance());
